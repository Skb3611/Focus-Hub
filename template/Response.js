  ...
  <Html>
    ...
    <Body style={{ fontFamily: 'Arial, sans-serif', color: '#333', lineHeight: 1.5, backgroundColor: '#f6f6f6', padding: '20px' }}>
      <Container style={{ maxWidth: '600px', margin: '0 auto', padding: '20px', backgroundColor: '#ffffff', borderRadius: '8px', boxShadow: '0 0 10px rgba(0,0,0,0.1)' }}>
        ...
        {['Name', 'Email', 'Phone', 'Date', 'Time', 'Category'].map((label, index) => (
          <Section style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px', width: '100%' }} key={index}>
            <Text style={{ fontSize: '16px', fontWeight: 'bold' }}>{label}:</Text>
            <Text style={{ fontSize: '16px' }}>{body[label.toLowerCase()]}</Text>
          </Section>
        ))}
