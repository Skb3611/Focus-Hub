...

    {
      ...
      desc: (
        <Link
          href={"/contact"}
          type="button"
          className=" text-white bg-gradient-to-br from-purple-600 to-blue-500 hover:bg-gradient-to-bl focus:ring-2 focus:outline-none focus:ring-blue-300 dark:focus:ring-blue-800 font-medium rounded-lg text-base px-5 py-2 text-center w-full"
        >
          Visit Contact Us
        </Link>
      ),
          ...
          {data.map((item) => {
            if (item.position == "left") {
              return (
                <div
                  key={item.title}
                  className="flex items-center lg:w-3/5 mx-auto border-1 pb-10 lg:mb-10 border-black sm:flex-row flex-col"
                >
                  ...
                  <div className="flex-grow sm:text-left text-center mt-6 sm:mt-0">
                    ...
                    <p className="leading-relaxed text-lg text-justify sm:text-left">
                      {item.desc}
                    </p>
                  </div>
                </div>
              ...
              return (
                <div
                  key={item.title}
                  className="flex flex-col sm:flex-row-reverse items-center lg:w-3/5 mx-auto border-b pb-10 mb-10 border-gray-200   gap-2"
                >
                  <div className="sm:w-32 sm:h-32 h-24 w-24 sm:mr-10 inline-flex items-center justify-center rounded-full bg-indigo-100 text-indigo-500 flex-shrink-0">
                    {item.icon}
                  </div>
